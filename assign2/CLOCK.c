...
int CLOCK_findByPageNumber(BM_BufferPool *const bm, int pageNum){
    mgmtDataCLOCK * nodes = (mgmtDataCLOCK * )bm->mgmtData;
    int i;
...
//init the mgmtDate into mgmtDataCLOCK (one kind of page frame)
RC initmgmtdataCLOCK(BM_BufferPool * const bm){
    mgmtDataCLOCK * nodes = (mgmtDataCLOCK *)malloc(bm->numPages*sizeof(struct mgmtDataCLOCK));
    int i;
    for(i=0;i<bm->numPages;i++){
        nodes[i].fix = 0;
        nodes[i].dirty = false;
...
RC CLOCK_makeDirty(BM_BufferPool *const bm, BM_PageHandle *const page){
    int i = CLOCK_findByPageNumber(bm, page->pageNum);
    if(i<0){
...
//set the dirty to false in the mgmtDataCLOCK
...
//And adding one to each time of mgmtDataCLOCK in the buffer pool
RC CLOCK_pinPage(BM_BufferPool *const bm, BM_PageHandle *const page, PageNumber pageNum){
    //printf("pin: %d\n", pageNum);
    int i;
    int ind = CLOCK_findByPageNumber(bm, pageNum);
    ...
    i = CLOCK_findTheEariestUnfixedNode(bm);
    if (i == -1){
